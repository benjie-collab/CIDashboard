...
class Document extends CI_Controller {
	...
	function index()
	{
		...
		$this->data['js'] 	 = array('js/documents.js');	
		...
		}else
		{
			...
			$this->data['main_content'] = 'document/index';
			$this->load->view('document/template', $this->data); 
		}
	...
	function datatable()
    {
        $this->datatables->select('id,file_name,file_type,file_size')
            //->unset_column('id')
			->filter('is_image', false)
			->filter('user_id', $this->users->get_user_id())
			->add_column('actions', get_buttons('$1', 'document',array('edit', 'delete')), 'id')
            ->from('uploads');			
	...
	function add()
	{
		...
		else
		{
			...
			if($method == 'POST'){				
				...
				$this->data['main_content'] = 'document/add';
				$this->load->view('document/template', $this->data);			
			}
		}
