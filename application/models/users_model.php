...
class Users_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->tables  = $this->config->item('tables', 'users');

		//initialize data
		$this->identity_column = $this->config->item('identity', 'users');
		$this->store_salt      = $this->config->item('store_salt', 'users');
		$this->salt_length     = $this->config->item('salt_length', 'users');
		$this->join			   = $this->config->item('join', 'users');


		//initialize hash method options (Bcrypt)
		$this->hash_method = $this->config->item('hash_method', 'users');
		$this->default_rounds = $this->config->item('default_rounds', 'users');
		$this->random_rounds = $this->config->item('random_rounds', 'users');
		$this->min_rounds = $this->config->item('min_rounds', 'users');
		$this->max_rounds = $this->config->item('max_rounds', 'users');
		...
		$this->messages    = array();
		$this->errors      = array();
		$delimiters_source = $this->config->item('delimiters_source', 'users');
		...
		else
		{
			...
			$this->message_start_delimiter = $this->config->item('message_start_delimiter', 'users');
			$this->message_end_delimiter   = $this->config->item('message_end_delimiter', 'users');
			$this->error_start_delimiter   = $this->config->item('error_start_delimiter', 'users');
			$this->error_end_delimiter     = $this->config->item('error_end_delimiter', 'users');
		...
		if ($this->hash_method == 'bcrypt') {
			...
			$params['salt_prefix'] = $this->config->item('salt_prefix', 'users');
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			...
			if ($query->num_rows() !== 1)
			{
				...
				$this->notification->set_error('activate_unsuccessful');
				return FALSE;
			}
		...
		if ($return)
		{
			...
			$this->notification->set_message('activate_successful');
		}
		else
		{
			$this->trigger_events(array('post_activate', 'post_activate_unsuccessful'));
	...
	public function deactivate($id = NULL)
	{
		...
		$return = $this->db->affected_rows() == 1;
		if ($return)
			$this->notification->set_message('deactivate_successful');
		else
			$this->notification->set_error('deactivate_unsuccessful');
		...
		if ($profile) {
			...
			if ($this->config->item('forgot_password_expiration', 'users') > 0) {
				...
				if (time() - $profile->forgotten_password_time > $expiration) {
					//it has expired
					$this->notification->set_error('forgot_password_expired');
					$this->trigger_events(array('post_forgotten_password_complete', 'post_forgotten_password_complete_unsuccessful'));
					return FALSE;
				}
			}
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$manual_activation = $this->config->item('manual_activation', 'users');

		if ($this->identity_column == 'email' && $this->email_check($email))
		{
			$this->notification->set_error('account_creation_duplicate_email');
			return FALSE;
		}
		elseif ($this->identity_column == 'username' && $this->username_check($username))
		{
			$this->notification->set_error('account_creation_duplicate_username');
			return FALSE;
		}
		elseif ( !$this->config->item('default_group', 'users') && empty($groups) )
		{
			$this->notification->set_error('account_creation_missing_default_group');
			return FALSE;
		}

		//check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'],array('name' => $this->config->item('default_group', 'users')),1)->row();
		if( !isset($query->id) && empty($groups) ) 
		{
			$this->notification->set_error('account_creation_invalid_default_group');
			return FALSE;
		}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		{
			$this->notification->set_error('login_unsuccessful');
			return FALSE;
		}
		...
		if($this->is_time_locked_out($identity))
		{
			...
			$this->notification->set_error('login_timeout');
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				{
					$this->trigger_events('post_login_unsuccessful');
					$this->notification->set_error('login_unsuccessful_not_active');
				...
				if ($remember && $this->config->item('remember_users', 'users'))
				{
					$this->remember_user($user->id);
				}

				$this->trigger_events(array('post_login', 'post_login_successful'));
				$this->notification->set_message('login_successful');
	...
	public function is_max_login_attempts_exceeded($identity) {
		if ($this->config->item('track_login_attempts', 'users')) {
			$max_attempts = $this->config->item('maximum_login_attempts', 'users');
			if ($max_attempts > 0) {
	...
	function get_attempts_num($identity)
	{
        if ($this->config->item('track_login_attempts', 'users')) {
            ...
            if ($this->config->item('track_login_ip_address', 'users')) $this->db->where('ip_address', $ip_address);
	...
	public function is_time_locked_out($identity) {
		...
		return $this->is_max_login_attempts_exceeded($identity) && $this->get_last_attempt_time($identity) > time() - $this->config->item('lockout_time', 'users');
	}
	...
	public function update($id, array $data)
	{
		...
		if (array_key_exists($this->identity_column, $data) && $this->identity_check($data[$this->identity_column]) && $user->{$this->identity_column} !== $data[$this->identity_column])
		{
			...
			$this->notification->set_error('account_creation_duplicate_'.$this->identity_column);

			$this->trigger_events(array('post_update_user', 'post_update_user_unsuccessful'));
			$this->notification->set_error('update_unsuccessful');
		...
		$this->notification->set_message('update_successful');
		return TRUE;
	}
	...
	**/
	public function delete_user($id)
	{
		...
		if ($this->db->trans_status() === FALSE)
		{
			...
			$this->notification->set_error('delete_unsuccessful');
			return FALSE;
		}
		...
		$this->notification->set_message('delete_successful');
		return TRUE;
	}
	...
	public function set_lang($lang = 'en')
	{
		...
		if($this->config->item('user_expire', 'users') === 0)
		{
		...
		{
			$expire = $this->config->item('user_expire', 'users');
		}
	...
	public function remember_user($id)
	{
		...
		if ($this->db->affected_rows() > -1)
		{
			...
			set_cookie(array(
			    'name'   => $this->config->item('identity_cookie_name', 'users'),
			    'value'  => $user->{$this->identity_column},
			    'expire' => $expire
			));

			set_cookie(array(
			    'name'   => $this->config->item('remember_cookie_name', 'users'),
			    'value'  => $salt,
			    'expire' => $expire
			));
	...
	public function login_remembered_user()
	{
		...
		if (!get_cookie($this->config->item('identity_cookie_name', 'users'))
			|| !get_cookie($this->config->item('remember_cookie_name', 'users'))
			|| !$this->identity_check(get_cookie($this->config->item('identity_cookie_name', 'users'))))
		{
		...
		$query = $this->db->select($this->identity_column.', id, username, email, last_login')
		                  ->where($this->identity_column, get_cookie($this->config->item('identity_cookie_name', 'users')))
		                  ->where('remember_code', get_cookie($this->config->item('remember_cookie_name', 'users')))
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
		...
		if ($query->num_rows() == 1)
		{
			...
			if ($this->config->item('user_extend_on_login', 'users'))
			{
				$this->remember_user($user->id);
			}
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		if(!$group_name)
		{
			$this->notification->set_error('group_name_required');
			return FALSE;
		}
		...
		if($existing_group !== 0)
		{
			$this->notification->set_error('group_already_exists');
			return FALSE;
		}
		...
		// report success
		$this->notification->set_message('group_creation_successful');
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name))
		{
			...
			if(isset($existing_group->id) && $existing_group->id != $group_id)
			{
				...
				$this->notification->set_error('group_update_notallowed');
				return FALSE;
			}
		...
		$this->notification->set_message('group_update_successful');
	...
	**/
	public function delete_group($group_id = FALSE)
	{
		...
		if($group->name == $this->config->item('admin_group', 'users'))
		{
			$this->trigger_events(array('post_delete_group', 'post_delete_group_notallowed'));
			$this->notification->set_error('group_delete_notallowed');
			return FALSE;
		}
		...
		if ($this->db->trans_status() === FALSE)
		{
			...
			$this->notification->set_error('group_delete_unsuccessful');
			return FALSE;
		}
		...
		$this->notification->set_message('group_delete_successful');
		return TRUE;
	}
