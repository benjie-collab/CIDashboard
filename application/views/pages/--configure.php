<?php 
	...
	$form_to_submit = '#page_configure_form';
?>
sdsd
				...
				<div id="<?=$id?>" class="connected-widgets widget widget-empty widget-sm"
					...
					jQuerySortable: {
						...
						update: function( event, ui ){							
							...
							localStorage.setItem('<?=$id?>', JSON.stringify(data)); localStorage.setItem('redirect', redirect);
							$(parent).siblings('.overlay:eq(0)').addClass('hidden');
							$('<?=$form_to_submit?>').trigger('submit');
						}
					}
					"
				>	
					<?php 
						if(element($id, $page)):
							foreach(element($id, $page) as $key=>$widget_key):
