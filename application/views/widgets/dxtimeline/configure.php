<?php 
	...
	$hidden = array(
				...
				'visual[commonSeriesSettings][argumentField]' => 'month',		
	...
	$timeline = $timeline? $timeline : elements(array('dataSource', 'series'), $config_visual);		
		...
		<div class="row">		
			<div class="col-md-8 col-sm-7">	
				<div id="statistics-preview-container" class="form-inline">
					<?php $this->load->view('/widgets/dxtimeline/view', array(
						'widget_key' => $widget_key, 
						'widgetize' => false,
						'timeline' =>$timeline,
						'event' => 'modal')
					...
					<div class="tab-pane" id="config_tab_3">
						<div data-bind="CustomScrollbar: {  						
							...
							<fieldset class="m-b-20">
								...
								<div class="input-group m-b-10">
									...
									<?php 
										...
										echo form_dropdown('visual[commonSeriesSettings][type]', element('basic', $av_types) , $value , $atts);												
									?>		
								</div>
								<div class="input-group m-b-10">
									<label for="labelVisible" class="input-group-addon">label</label>						
									<div class="form-control p-5">
									<?php	
									$label =  element('label', $commonSeriesSettings);	
									$label =  is_array($label)? $label : array();
														
									$data = array(
									'name'        => 'visual[commonSeriesSettings][label][visible]',
									'id'          => 'labelVisible',
									'value'       => 1,
									'checked'     => (bool)element('visible', $label) == true,
									'style'       => 'margin:10px',
									'class'		  => 'm-0 p-absolute',
							...
							<fieldset class="m-b-20">
								...
								<?php $title = array_key_exists('title', $argumentAxis)? element('title', $argumentAxis) : array();?>	
								<div class="form-group">
									<label for="title">Title</label>							
									<?php $data = array(
												  'name'        => 'visual[argumentAxis][title][text]',
												  'id'          => 'title',
												  'maxlength'   => '30',
												  'class'		=> 'form-control',
												  'value'		=> element('text', $title),												  
												);
									echo form_input($data); ?>														   
								</div>
								<div class="input-group m-b-10">
							...
							<fieldset class="m-b-20">
								...
								<?php $title = array_key_exists('title', $valueAxis)? element('title', $valueAxis) : array();?>	
								<div class="form-group">
									<label for="title">Title</label>							
									<?php $data = array(
												  'name'        => 'visual[valueAxis][title][text]',
												  'id'          => 'title',
												  'maxlength'   => '30',
												  'class'		=> 'form-control',
												  'value'		=> element('text', $title),												  
							...
							<fieldset class="m-b-20">
								<legend><i class="fa fa-bar-chart"></i> Bar Specific</legend>
								<?php $equalBarWidth = array_key_exists('equalBarWidth', $config_visual)? element('equalBarWidth', $config_visual) : array();?>
								<div class="input-group m-b-10">
									<label for="equalBarWidthWidth" class="input-group-addon">Bar Width</label>	
									<?php	
									$value =  element('width', $equalBarWidth);	
														
									$data = array(
										  'name'        => 'visual[equalBarWidth][width]',
										  'id'          => 'equalBarWidthWidth',							  
										  'class'		=> 'form-control',
										  'value'		=> $value,
										  'type'		=> 'number'
										);
								...
								<div class="input-group m-b-10">
									<label for="equalBarWidthSpacing" class="input-group-addon">Bar Spacing</label>	
									<?php	
									$value =  element('spacing', $equalBarWidth);	
									$data = array(
										  'name'        => 'visual[equalBarWidth][spacing]',
										  'id'          => 'equalBarWidthSpacing',							  
										  'class'		=> 'form-control',
										  'value'		=> $value,
										  'type'		=> 'number'
										);
