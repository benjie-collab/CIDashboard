ko.bindingHandlers.Dxtimeline = {
	...
	update: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
		...
		function initialize(settings){
			...
			if( visual_options.dataSource.length >0)
			jQuery(element).dxChart('instance').option(visual_options);
			else{
				jQuery(element).dxChart('instance').option(opts.visual);
			}
