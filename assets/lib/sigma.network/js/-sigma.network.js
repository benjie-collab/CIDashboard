    ...
    $.fn.SigmaNetwork = function(options) {
		...
		return this.each( function() {
			...
			self.configSigmaElements = function() {
							...
							var a = [],
								b,x=1;
								for (b in self.sigInst.clusters) a.push('<li style="line-height:12px"><a href="#' + b + '"><div style="width:12px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div> Group ' + (x++) + ' (' + self.sigInst.clusters[b].length + ' members)</a></li>');
							//a.sort();
							...
							default:
							 ...
							 CustomShapes.init(self.sigInst);
			...
			self.nodeActive = function (a) {
							...
							d = "";
								for (g in e) {
									...
									f.push('<li class="membership"><a class="cluster" data-id="' + c.id + '" href="#' + c.name + '" onnmouseover="self.sigInst._core.plotter.drawHoverNode(self.sigInst._core.graph.nodesIndex[\'' + c.id + '\'])\" oncclick=\"nodeActive(\'' + c.id + '\')" onnmouseout="self.sigInst.refresh()">' + c.name + "</a></li>");
								}
								return f;
							}
							...
							if (f.attributes) {
								...
								if (image_attribute) {
									...
									self._GP.info_name.html("<div><img src=" + f.attributes[image_attribute] + " style=\"vertical-align:middle\" /> <span onnmouseover=\"self.sigInst._core.plotter.drawHoverNode(self.sigInst._core.graph.nodesIndex['" + b.id + '\'])" onnmouseout="self.sigInst.refresh()">' + b.label + "</span></div>");
								} else {
									self._GP.info_name.html("<div><span onnmouseover=\"self.sigInst._core.plotter.drawHoverNode(self.sigInst._core.graph.nodesIndex['" + b.id + '\'])" onnmouseout="self.sigInst.refresh()">' + b.label + "</span></div>");
								}
			...
			self.showCluster = function (a) {
							...
							if (b && 0 < b.length) {
								...
								for (var f = [], e = [], c = 0, g = b.length; c < g; c++) {
									...
									!0 == d.hidden && (e.push(b[c]), d.hidden = !1, d.attr.lineWidth = !1, d.attr.color = d.color, f.push('<li class="membership"><a class="cluster" data-id="' + d.id + '" href="#'+d.label+'" onnmouseover="self.sigInst._core.plotter.drawHoverNode(self.sigInst._core.graph.nodesIndex[\'' + d.id + "'])\" oncclick=\"nodeActive('" + d.id + '\')" onnmouseout="self.sigInst.refresh()">' + d.label + "</a></li>"))
								}
			...
			self._GP.search = new self.Search(self._GP.form.find("#sigma_network_search"));
			...
			$(document)
			.on('click', '.membership .cluster', function(){ 
				var id= $(this).data('id'); 
				self.nodeActive(id); 
			});
			$(document)
			.on('mouseover', '.membership .cluster', function(){ 
				var id= $(this).data('id');
				self.sigInst._core.plotter.drawHoverNode(self.sigInst._core.graph.nodesIndex[id]);
			});
			$(document)
			.on('mouseout', '.membership .cluster', function(){ 
				self.sigInst.refresh();
			})
